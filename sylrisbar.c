...
set_ewmh_atoms (void)
{
    ...
    for (monitor_t *mon = monhead; mon; mon = mon->next) {
        ...
        xcb_change_property(c, XCB_PROP_MODE_REPLACE, mon->window, XCB_ATOM_WM_CLASS, XCB_ATOM_STRING, 8, 12, "sylrisbar\0Bar");
    }
}
...
main (int argc, char **argv)
{
    ...
    while ((ch = getopt(argc, argv, "hg:bdf:a:pu:B:F:U:n:o:")) != -1) {
        switch (ch) {
            case 'h':
                printf ("sylrisbar version %s patched with XFT support\n", VERSION);
